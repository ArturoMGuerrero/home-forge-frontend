...
export function CompanyProfileSettingsPage() {
  ...
  async function submit(event: FormEvent) {
    ...
    try {
      ...
      setForm(profile);
      setSuccess('Perfil empresarial actualizado. Los cambios ya aparecen en el sitio público.');
    } catch (requestError) {
      setError(requestError instanceof Error ? requestError.message : 'No fue posible guardar el perfil.');
    } finally {
      setSaving(false);
    }
  }
    ...
    <>
      ...
      <form className="grid gap-6 xl:grid-cols-[1fr_340px]" onSubmit={submit}>
        ...
        <aside className="space-y-5">
          ...
          {success && <p className="rounded-xl bg-emerald-50 p-4 text-sm font-medium text-emerald-700">{success}</p>}
