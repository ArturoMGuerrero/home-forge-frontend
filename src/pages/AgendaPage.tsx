...
export function AgendaPage() {
  ...
  useEffect(() => {
    Promise.all([listAppointments(), loadOperationsContext()])
      ...
      .catch(requestError => setError(requestError instanceof Error ? requestError.message : 'No fue posible cargar la agenda.'));
  }, []);
  ...
  async function submit(event: FormEvent) {
    ...
    try {
      ...
      setError(requestError instanceof Error ? requestError.message : 'No fue posible guardar la cita.');
  ...
  async function remove(id: string) {
    ...
    try {
      ...
      setError(requestError instanceof Error ? requestError.message : 'No fue posible eliminar la cita.');
    }
  }
    ...
    <>
      ...
      <div className="grid gap-6 xl:grid-cols-[380px_1fr]">
        ...
        <section className="space-y-5">
          {Object.entries(grouped).map(([date, items]) => <div className="rounded-2xl border border-slate-200 bg-white p-5 shadow-sm" key={date}><h2 className="mb-3 capitalize font-bold text-slate-700">{date}</h2>{items.map(item => <article className="flex flex-wrap items-center gap-4 border-t border-slate-100 py-4 first:border-0" key={item.id}><time className="w-20 text-sm font-bold text-indigo-700">{new Date(item.startsAt).toLocaleTimeString('es-MX', { hour: '2-digit', minute: '2-digit' })}</time><div className="min-w-0 flex-1"><strong className="block">{item.title}</strong><p className="text-sm text-slate-500">{item.location || 'Sin ubicación'}{item.notes ? ` · ${item.notes}` : ''}</p></div><span className={`rounded-full px-3 py-1 text-xs font-bold ${item.status === 'COMPLETED' ? 'bg-emerald-100 text-emerald-800' : item.status === 'CANCELED' ? 'bg-slate-200 text-slate-700' : 'bg-indigo-100 text-indigo-800'}`}>{item.status}</span><div className="flex gap-2">{item.status === 'SCHEDULED' && <button className="rounded-lg border border-slate-200 px-3 py-2 text-xs font-bold hover:bg-slate-50" onClick={() => complete(item)}>Marcar realizada</button>}<button className="rounded-lg border border-rose-200 bg-rose-50 px-3 py-2 text-xs font-bold text-rose-700 hover:bg-rose-100" onClick={() => remove(item.id)}>Eliminar</button></div></article>)}</div>)}
